<%@ page language="java" contentType="text/html; charset=EUC-KR"
	pageEncoding="EUC-KR"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=EUC-KR">
<title>사이트맵</title>
</head>
<body>
	<div style="float: left;">
		<span>자료신청 및 예약</span>
		<ul>
			<li><a href="/rent/rent.do">대출연기및반납</a></li>
			<li>예약연기및반납</li>
			<li>희망도서신청</li>
		</ul>
	</div>

	<div style="float: left;">
		<span>자료검색</span>
		<ul>
			<li><a href="/search/search.do">자료검색</a></li>
			<li><a href="/search/newbook.do">신간자료</a></li>
			<li><a href="/search/popularbook.do">인기자료</a></li>
		</ul>
	</div>
	
	<div style="float: left;">
		<span>참여마당</span>
		<ul>
			<li><a href="/notice/notice.do">공지사항</a></li>
			<li><a href="/lostBoard/list.do">분실물게시판</a></li>
		</ul>
	</div>
	
	<div style="float: left;">
		<span><a href="/userguide/guide.do">이용안내</a></span>
		<ul>
			<li><a href="/userguide/library_map.do">이용안내</a></li>
			<li><a href="/userguide/library_time.do">도서관이용시간</a></li>
		</ul>
	</div>
	
	<div style="float: left;">
		<span><a href="/mypage/mypage.do">마이페이지</a></span>
		<ul>
			<li><a href="mypage/mybook.do">나의관심도서</a></li>
			<li><a href="/mypage/member_pcheck.do">회원정보수정</a></li>
			<li><a href=/mypage/member_leave.do">회원탈퇴</a></li>
			<li><a href="/mypage/rental_guide.do">대출내역</a></li>
			<li><a href="mypage/reserve_guide.do">예약내역</a></li>
			<li><a href="/mypage/rec_select.do">한줄평모음</a></li>
		</ul>
	</div>
</body>
</html>